export class AircraftSeating {
    ...
    bookSeat(rowNumber, column) {
        if (rowNumber < 1 || rowNumber > this.numRows) {
            return "Invalid row number.";
        }

        const side = column in {'A': 1, 'B': 1, 'C': 1} ? 'left' : 'right';
    ...
    checkSeatAvailability(rowNumber, column) {
        if (rowNumber < 1 || rowNumber > this.numRows) {
            return "Invalid row number.";
        }
            ...
            for (const side of ['left', 'right']) {
                ...
                for (let i = 0; i <= columns.length - numSeats; i++) {
                    if (columns.slice(i, i + numSeats).every(col => !row[side][col])) {
                        if (!this.wouldLeaveSingleScatteredSeat(columns, i, i + numSeats, row[side])) {
            ...
            for (const side of ['left', 'right']) {
                ...
                for (const col of columns) {
                    ...
                    if (availableSeats.length === numSeats) {
                        if (!this.wouldLeaveSingleScatteredSeat(columns, availableSeats[0][1], availableSeats[availableSeats.length - 1][1] + 1, row[side])) {
                            availableSeats.forEach(([r, c]) => {
                                const s = c in {'A': 1, 'B': 1, 'C': 1} ? 'left' : 'right';
    ...
    wouldLeaveSingleScatteredSeat(rowIndex, column, side) {
    ...
    wouldLeaveSingleScatteredSeat(columns, start, end, rowSide) {
