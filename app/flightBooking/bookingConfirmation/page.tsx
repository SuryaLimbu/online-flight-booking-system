      ...
      <div className="flex-cols sm:flex gap-8">
        <div className="flex-cols w-full">
          ...
          {bookingData.map((booking) => (
            <div key={booking._id} className="space-y-4 p-4 border rounded-b-xl">
              <div>
                <h1>Boarding Pass</h1>
                <h1>Booking Id: {booking._id}</h1>
              </div>
              <div className="space-y-4">
                {booking.flightId && booking.flightId.departureAirport && booking.flightId.arrivalAirport && (
                  <>
                    <div className="flex justify-between items-center">
                      <div>
                        <span>{booking.flightId.departureAirport.location}</span>
                        <br />
                      ...
                      <div>
                        <span>{booking.flightId.arrivalAirport.location}</span>
                        <br />
                        <span className="font-bold text-lg sm:text-2xl">
                    ...
                    <div className="flex justify-between">
                      <div>
                        ...
                        <br />
                        <span className="font-bold text-lg sm:text-2xl">
                          {new Date(booking.flightId.departureTime).toLocaleTimeString()}
                        </span>
                      </div>
                      <div>
                        <span>Duration</span>
                        <br />
                        <span className="font-bold text-lg sm:text-2xl">
                          {calculateDuration(booking.flightId.departureTime, booking.flightId.arrivalTime)}
                        </span>
                      </div>
                      <div>
                        <span>Arrival</span>
                        <br />
                        <span className="font-bold text-lg sm:text-2xl">
                          {new Date(booking.flightId.arrivalTime).toLocaleTimeString()}
                        </span>
                      </div>
                    </div>
                    <hr />
                    <div className="flex justify-between text-xl gap-4">
                      <div className="p-4 rounded-lg">
                        ...
                        <span className="font-bold text-lg sm:text-2xl text-primary">
                          {booking.seatId.map((seat) => seat.seatName).join(", ")}
                        </span>
                      </div>
                      <div className="p-4 rounded-lg">
                        <h1>Flight</h1>
              ...
              <hr />
              <div>
                <h1 className="uppercase text-xl sm:3xl font-semibold">Passengers</h1>
                <ul className="py-4 space-y-4">
                  {booking.passengers.map((passenger) => (
                    <li key={passenger._id} className="flex items-center gap-4">
                      ...
                      <span className="text-lg sm:text-xl text-primary">{passenger.fullName}</span>
              ...
              <div>
                <h1 className="uppercase text-xl sm:3xl font-semibold">Contact Details</h1>
              </div>
            </div>
          ))}
        </div>
        <div className="sm:w-2/4 space-y-4">
          <div className="border p-4 rounded-xl">
            <h1 className="font-semibold text-lg sm:text-2xl text-primary">Amount Paid</h1>
            <p className="flex justify-between">
              <span className="font-semibold">{totalPassengers} passengers</span>
              <span className="text-primary font-semibold">£ {bookingData[0].totalPrice}</span>
            </p>
            <p>includes taxes</p>
          </div>
          <div className="border p-4 rounded-xl">
            <h1 className="font-semibold text-lg sm:text-2xl text-primary">Airlines Contact</h1>
            <p>
